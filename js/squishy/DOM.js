Module(function M() { if(document) {
  ...
  var TagList=M.Class(function C() {
    C.Init(function TagList() {

      Object.defineProperty(this, "length",{editable:true,value:0})

    });
    C.Def(function forEach(callback) {
  ...
  var Tag=M.Class(function C() {
    ...
    C.Def(function removeClass(cls) {
      var clss;
      clss=this.element.getAttribute('class');
      if(clss&&clss.split) clss=clss.split('/\W+/');//Name.split(' ');
      else clss=[];
      var i=clss.indexOf(cls);
      if(i>-1) {
        delete clss[i];
